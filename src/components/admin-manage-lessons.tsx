    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>{DELETE_LESSON_CONFIRM[lang]}</AlertDialogDescription>
        </AlertDialogHeader>
        <p className="text-sm font-medium text-foreground">{lesson.title[lang]}</p>
        <AlertDialogFooter>
                ...
                {lessons.map((l) => (
                  <tr key={l.id} className="border-b border-border/60 last:border-0">
                    <td className="py-3 px-2">
                      <div className="font-medium text-foreground">{l.title[lang]}</div>
                      <div className="text-xs text-muted-foreground truncate max-w-[220px]">
                        {l.title[lang === "en" ? "ar" : "en"]}
                      </div>
                    </td>
                    <td className="py-3 px-2 text-muted-foreground whitespace-nowrap">
                      {grades.find((g) => g.slug === l.grade)?.name[lang] ?? l.grade}
                    </td>
