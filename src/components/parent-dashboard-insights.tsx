...
export function ParentDashboardInsights({ data }: { data: ParentDashboardData }) {
  ...
  const insights = buildParentInsights(data.performanceReport, data.progress, tr, trf);
  ...
  return (
    <section className="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/8 via-card to-card p-5 shadow-[var(--shadow-soft)] sm:p-6">
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <h2 className="font-display text-xl text-foreground">{tr("parent_summary_title")}</h2>
      </div>

      {insights.length === 0 ? (
        <p className="text-sm italic text-muted-foreground">{tr("parent_summary_empty")}</p>
      ) : (
