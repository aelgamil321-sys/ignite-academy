...
import {
  isParentAchievementsNav,
  isParentAnnouncementsNav,
  isParentAssignmentsNav,
  isParentDashboardNav,
  isParentGuidesNav,
  isParentProgressNav,
  PARENT_NAV_ANCHORS,
  scrollToParentAnchor,
} from "@/lib/parent-nav";
...
function SidebarFooter({
  ...
  const logout = async () => {
    ...
    window.location.assign("/auth?mode=login&accountType=parent");
  };
...
export function ParentSidebar({
  ...
  const { displayName, email } = useParentShell();
  const closeMobile = () => onMobileOpenChange?.(false);

  const navItems: NavItem[] = [
    {
      key: "dashboard",
      labelKey: "parent_nav_dashboard",
      icon: LayoutDashboard,
      to: "/parent/dashboard",
      exact: true,
      activeMatch: (pathname, hash) => isParentDashboardNav(pathname, hash),
    },
    {
      key: "progress",
      labelKey: "parent_nav_child_progress",
      icon: TrendingUp,
      to: "/parent/dashboard",
      hash: PARENT_NAV_ANCHORS.progress,
      activeMatch: (pathname, hash) => isParentProgressNav(pathname, hash),
      onNavigateClick: (pathname, hash) => {
        if (isParentProgressNav(pathname, hash)) {
          scrollToParentAnchor(PARENT_NAV_ANCHORS.progress);
        }
      },
    },
    {
      key: "assignments",
      labelKey: "parent_nav_assignments",
      icon: ClipboardCheck,
      to: "/parent/dashboard",
      hash: PARENT_NAV_ANCHORS.assignments,
      activeMatch: (pathname, hash) => isParentAssignmentsNav(pathname, hash),
      onNavigateClick: (pathname, hash) => {
        if (isParentAssignmentsNav(pathname, hash)) {
          scrollToParentAnchor(PARENT_NAV_ANCHORS.assignments);
        }
      },
    },
    {
      key: "achievements",
      labelKey: "parent_nav_achievements",
      icon: Award,
      to: "/parent/dashboard",
      hash: PARENT_NAV_ANCHORS.achievements,
      activeMatch: (pathname, hash) => isParentAchievementsNav(pathname, hash),
      onNavigateClick: (pathname, hash) => {
        if (isParentAchievementsNav(pathname, hash)) {
          scrollToParentAnchor(PARENT_NAV_ANCHORS.achievements);
        }
      },
    },
    {
      key: "announcements",
      labelKey: "parent_nav_announcements",
      icon: Megaphone,
      to: "/parent/announcements",
      activeMatch: (pathname) => isParentAnnouncementsNav(pathname),
    },
    {
      key: "guides",
      labelKey: "parent_nav_guides",
      icon: BookOpen,
      to: "/parent/guides",
      activeMatch: (pathname) => isParentGuidesNav(pathname),
    },
    {
      key: "profile",
      labelKey: "parent_nav_profile",
      icon: User,
      to: "/parent/settings",
      exact: true,
      activeMatch: (pathname) => pathname === "/parent/settings" || pathname === "/parent/settings/",
    },
  ];
