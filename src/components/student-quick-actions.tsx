...
export function StudentQuickActions() {
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-brand-dark/25 bg-brand-dark p-4 shadow-[var(--shadow-soft)] sm:p-5">
      <h3 className="mb-3 font-display text-base font-semibold text-white sm:text-lg">
        ...
        {actions.map((action) => {
          ...
          return (
            <Link
              ...
              className={cn(
                "group flex min-h-[4.5rem] min-w-0 flex-col items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/8 p-2.5 text-center transition-colors hover:border-[#FF7A00]/55 hover:bg-white/12 sm:min-h-[5rem] sm:p-3",
              )}
            >
              ...
              <span className="line-clamp-2 min-w-0 text-[11px] font-semibold leading-snug text-white group-hover:text-[#FF7A00] sm:text-xs">
                {tr(action.labelKey)}
              </span>
            </Link>
          );
